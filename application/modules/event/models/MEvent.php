 ...
 * @author          Barun Pandey
 * @date            20 March, 2020, 05:55:00 PM
 * @version         1.0.0
 */
class mEvent extends CI_Model {
    ...
    public function getEvent($id = 0) {
        $result = $this->callCurl('GET', '', 'http://localhost/displayfort-api/api/event/event/' . $id . '');
    ...
    public function getAllEvent() {
        $result = $this->callCurl('GET', '', 'http://localhost/displayfort-api/api/event/event/');
        if (empty($result)) {
            echo "something get error please try after some time";
        } else {
            return($result);
    ...
    public function addEvent($data) {
        $result = $this->callCurl('POST', json_encode($data), 'http://localhost/displayfort-api/api/event/event/');
            print_r($result);
        if (empty($result)) {
            echo "something get error please try after some time";
        } else {
    ...
    public function deleteAssets($id) {
        $result = $this->callCurl('DELETE', '', 'http://localhost/displayfort-api/api/event/event/' . $id . '');
        if (empty($result)) {
            echo "something get error please try after some time";
        } else {
            print_r($result);
