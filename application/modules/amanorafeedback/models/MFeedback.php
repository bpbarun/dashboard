...
class mFeedback extends CI_Model {
    ...
    public function getFeedback($id = 0) {
        if (!(empty($id)) && is_numeric($id)) {
            $result = $this->callCurl('GET', '', 'http://localhost/displayfort-api/api/feedback-amanora/feedback/' . $catID);
        } else {
            $result = $this->callCurl('GET', '', 'http://localhost/displayfort-api/api/feedback-amanora/feedback/');
        }
        if (empty($result)) {
     ...
     * @return Response
     */
    public function getAllFeedbackQues() {
        // $id = $data['feedback_question_id'];
        $result = $this->callCurl('GET','', 'http://localhost/displayfort-api/api/feedback-amanora/FeedbackQuestion/');
        if (empty($result)) {
            echo "something get error please try after some time";
        } else {
            // print_r($result);
