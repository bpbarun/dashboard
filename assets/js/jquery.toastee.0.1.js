(function ($) {
    ...
    $.fn.toastee = function (options, type) {
...
//                settings.header = options.header || headers.success;
        ...
        var toast = '<div data-id="' + dataId + '" style="display: block; position: relative; border-radius: 10px; min-width: 380px; max-width: ' + settings.width + 'px; height: ' + settings.height + 'px; background: ' + settings.background + '; box-shadow: 0 5px 5px 2px #ccc">';
        toast += '<a class="closeToastee" style="color: ' + settings.color + '; text-align: center; line-height: .8; width: 31px; height: 26px; padding: 5px; border-radius: 100%; border: 2px solid ' + settings.color + '; font-family: sans-serif; display: block; cursor: pointer; position: absolute; top: 10px; right: 10px; color: ' + settings.color + ';">X</a>';
